...
class SFAPI:
  ...
  def login(self, login_url, max_retries=3, retry_delay=2):
    ...
    for attempt in range(1, max_retries + 1):
      ...
      try:
        proxies = {'http': self.proxy, 'https': self.proxy} if hasattr(self, 'proxy') and self.proxy else None
        ress = self.s.get(login_url, headers=self.get_headers("1"), timeout=10, proxies=proxies,
  ...
  def query_free_coupon(self):
    ...
    try:
      ...
      if response.get('success') and response.get('obj'):
        keywords = ["免单", "15元运费券"]
        filtered_coupons = [coupon for coupon in response['obj'] if
                            any(keyword in coupon.get('couponName', '') for keyword in keywords)]
        if filtered_coupons:
          for coupon in filtered_coupons:
            log.info(
              f"{coupon.get('couponName')}：{coupon.get('couponNum', 0)}张，过期时间：{coupon.get('invalidTm')}")
    ...
    try:
      ...
      if response.get('success') == True:
        ...
        for order in response['obj']['unpaidOrderDetail']:
          orders.append(
            {"waybillNo": order.get('waybillNo', ''), "amount": float(order.get('amt', '0')) / 100})
  ...
  def anniversary2025_draw_card(self):
    ...
    try:
      ...
      if response.get('success') == True:
        balance = next(
          (account.get('balance', 0) for account in response.get('obj', {}).get('currentAccountList', []) if
           account.get('currency') == 'CLAIM_CHANCE'), 0)
          ...
          try:
            ...
            if response.get('success') == True:
              ...
              for item in received_list:
                rewards.append(
                  f"{card_names.get(item.get('currency'), item.get('currency'))} × {item.get('amount')}")
      ...
      if final_response:
        ...
        card_info = [f"{card_names.get(card.get('currency'), card.get('currency'))} × {card.get('balance')}" for
                     card in current_cards if card.get('currency') != 'CLAIM_CHANCE']
        log.info("----- 周年庆卡片信息 -----\n" + "\n".join(card_info))
  ...
  def csy2025(self):  ##############################################################################
    ...
    try:
      ...
      self.do_request(
        ...
        {"inviteType": 4, "inviteUserId": random_invite})
  ...
  def dsl2025(self):
    ...
    try:
      random_invite = random.choice([invite for invite in inviteId if invite != self.user_id])
      self.do_request(
        "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonNoLoginPost/~memberNonactivity~disneyService~index",
        {"inviteType": 0, "inviteUserId": random_invite})
      ...
      if isinstance(response, dict) and response.get('success'):
        ...
        for task in tasks:
          ...
          if taskType not in ['BROWSE_LIFE_SERVICE', 'INTEGRAL_EXCHANGE', 'BROWSE_VIP_CENTER',
                              'LOOK_BRAND_VIDEO', 'BROWSE_STUDENT_BENEFIT']:
            continue
          print(f"> 正在处理任务【{taskName}】")
          if taskStatus == 3:
    ...
    try:
      ...
      if isinstance(response, dict) and response.get('success'):
        ...
        for i in range(cs):
          ...
          if draw_response.get('success') and draw_response.get('obj'):
            ...
            if draw_response.get('obj').get('paidProductPacket', {}):
              print(
                f"拆快递成功: 获得 {draw_response.get('obj').get('paidProductPacket', {}).get('productName', '-')}")
            elif draw_response.get('obj').get('award', {}):
              print(
                f"拆快递成功: 获得 {draw_response.get('obj').get('award', {}).get('productName', '-')}")
            else:
        ...
        response = self.do_request(
          ...
          {"tag": "DISNEY", "productType": "", "pageNo": 1, "pageSize": 200})
  ...
  def yearEnd2025_init(self):
    ...
    try:
      ...
      random_invite = random.choice([invite for invite in inviteId if invite != self.user_id])
      self.do_request(
        "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonNoLoginPost/~memberNonactivity~yearEnd2025IndexService~index",
        {"inviteType": 1, "inviteUserId": random_invite})
  ...
  def yearEnd2025_get_forward_status(self):
    ...
    try:
      ...
      if response.get('success'):
        ...
        print(
          f'抽奖状态: 已抽 {current_times}/{total_times} 次, 剩余 {remain_chance} 次机会, 当前进度: {current_ratio}%')
